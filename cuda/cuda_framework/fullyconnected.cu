...
FullyConnected::FullyConnected(std::unique_ptr<Matrix>& weight_set, std::unique_ptr<Matrix>& bias_set, float(*function)(float)) {
...
//	std::unique_ptr<Matrix> outApplied = out->apply(FullyConnected::activation_func);
//}
//
