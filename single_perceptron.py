...
class Brain:
    ...
    def feedForward(self, inputs):
        raw = resources.dot(inputs, self.weights) + self.bias
        return resources.sigmoid(raw)
    ...
    def trainNetwork(self, input_data, training_data):
        ...
        error = 0.5*(predicted - training_data) 

        for i in range(len(self.weights)):
            update = error*resources.sigmoid(predicted, deriv=True)*input_data[i]
            self.weights[i] -= 0.5*update

        biasUpdate = error*resources.sigmoid(predicted, deriv=True)
